...
class System():
    ...
    def latticebcc(self):
        self.closen = 0.
        self.energy = self.bcccell0()
        for i in np.linspace(-self.n, self.n, 2.*self.n+1):
            for j in np.linspace(-self.n, self.n, 2.*self.n+1):
                for k in np.linspace(-self.n, self.n, 2.*self.n+1):
                    if (i,j,k) != (0,0,0): self.energy += self.bcccell(self.a*i, self.a*j, self.a*k)
    ...
    def sccell(self, x, y, z):
        return self.potential(self.distanza(x, y, z))

    def fcccell(self, x, y, z):
        a = 2.**0.5
        return self.potential(self.distanza(a*x+(y+z)/a + self.noise*np.random.normal(), y/a + self.noise*np.random.normal(), z/a + self.noise*np.random.normal()))

    def bcccell(self, x, y, z):
        a = 1./(3.**0.5)
        return self.potential(self.distanza(2.*a*x, 2.*a*y, 2.*a*z)) + self.potential(self.distanza(2.*a*x+self.a*a, 2.*a*y+self.a*a, 2.*a*z+self.a*a))

    def hcpcell(self, x, y, z):
        return self.potential(self.distanza(x + y/2. + self.noise*np.random.normal(), y/2.*(3.**0.5)+self.noise*np.random.normal(), 24.**0.5*z/3.+self.noise*np.random.normal())) + self.potential(self.distanza(x+(y+self.a)/2.+self.noise*np.random.normal(), (y+self.a/3.)/2.*(3.**0.5)+self.noise*np.random.normal(), 24.**0.5*(z+self.a/2.)/3.+self.noise*np.random.normal()))
    ...
    def bcccell0(self):
        a = 1./2.**0.5
        return self.potential(self.distanza(self.a*a, self.a*a, self.a*a))

    def hcpcell0(self):
        return self.potential(self.distanza(self.a/2.+self.noise*np.random.normal(), self.a/3./2.*(3.**0.5)+self.noise*np.random.normal(), 6.**0.5*self.a/3.+self.noise*np.random.normal()))
